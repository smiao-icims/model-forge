...
class ModelForgeRegistry:
    ...
    def _get_model_config(
        ...
        if not provider_name or not model_alias:
            raise ConfigurationError("Could not determine provider and model to use.")
    ...
    def get_llm(
        ...
        try:
            ...
            if not creator:
                raise ProviderError(
                    f"Unsupported llm_type '{llm_type}' for provider '{resolved_provider}'"
                )

            return creator(resolved_provider, resolved_model, provider_data, model_data)

        except (ConfigurationError, ProviderError, ModelNotFoundError) as e:
            ...
            raise ProviderError("An unexpected error occurred during LLM creation.") from e
    ...
    def _create_openai_compatible_llm(
        ...
        if not api_key:
            raise ProviderError(f"Could not find token or key for provider: {provider_name}")
    ...
    def _create_ollama_llm(
        ...
        if not base_url:
            raise ConfigurationError(
                "Ollama 'base_url' not set in config and OLLAMA_HOST env var is not set."
            )
