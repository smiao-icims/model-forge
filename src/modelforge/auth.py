...
class ApiKeyAuth(AuthStrategy):
    ...
    def authenticate(self) -> Dict[str, Any]:
        ...
        """
        logger.info("Starting API key authentication for provider: %s", self.provider_name)
        ...
        try:
            ...
            logger.error("Failed to store API key for provider %s: %s", self.provider_name, str(e))
            raise AuthenticationError(f"Failed to store API key for {self.provider_name}") from e
    ...
    def get_credentials(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to retrieve API key for provider %s: %s", self.provider_name, str(e))
            return None
...
class DeviceFlowAuth(AuthStrategy):
    ...
    def authenticate(self) -> Dict[str, Any]:
        ...
        """
        logger.info("Starting device authentication flow for provider: %s", self.provider_name)
        ...
        try:
            ...
            logger.error("HTTP error requesting device code for %s: %s", self.provider_name, str(e))
            raise AuthenticationError(f"Failed to request device code for {self.provider_name}") from e
        except requests.exceptions.JSONDecodeError as e:
            logger.error("Invalid JSON response from device code request for %s", self.provider_name)
            raise AuthenticationError(f"Invalid response from {self.provider_name} device code endpoint") from e
        except requests.exceptions.RequestException as e:
            logger.error("Network error requesting device code for %s: %s", self.provider_name, str(e))
            raise AuthenticationError(f"Network error connecting to {self.provider_name}") from e
        ...
        while True:
            ...
            try:
                ...
                logger.error("Invalid JSON response while polling for token from %s", self.provider_name)
                raise AuthenticationError(f"Invalid response from {self.provider_name} token endpoint") from e
            except requests.exceptions.HTTPError as e:
                # Check if this is a recoverable error
                try:
                    error_info = token_response.json()
                    if error_info.get("error") == "authorization_pending":
                        logger.debug("Authorization pending for %s, continuing to poll", self.provider_name)
                        continue
                    elif error_info.get("error") == "slow_down":
                        logger.debug("Rate limited while polling for %s, slowing down", self.provider_name)
                        time.sleep(5)
                        continue
                    else:
                        logger.error("Authentication failed for %s: %s", self.provider_name, error_info.get("error"))
                        raise AuthenticationError(f"Authentication failed for {self.provider_name}: {error_info.get('error_description', 'Unknown error')}") from e
                except requests.exceptions.JSONDecodeError:
                    logger.error("HTTP error while polling for token from %s", self.provider_name)
                    raise AuthenticationError(f"HTTP error while polling for token from {self.provider_name}") from e
            except requests.exceptions.RequestException as e:
                logger.error("Network error while polling for token from %s: %s", self.provider_name, str(e))
                raise AuthenticationError(f"Network error while polling for token from {self.provider_name}") from e
            ...
            if "access_token" in token_data:
                ...
                try:
                    ...
                    logger.info("Successfully authenticated and stored token for %s", self.provider_name)
                    print(f"Successfully authenticated and stored token for {self.provider_name}.")
                    return {"access_token": token_data["access_token"]}
                except Exception as e:
                    logger.error("Failed to store token for %s: %s", self.provider_name, str(e))
                    raise AuthenticationError(f"Failed to store token for {self.provider_name}") from e
            elif token_data.get("error") == "authorization_pending":
                logger.debug("Authorization still pending for %s, continuing to poll", self.provider_name)
                continue
                ...
                logger.error("Failed to get access token for %s: %s", self.provider_name, error_desc)
                raise AuthenticationError(f"Failed to get access token for {self.provider_name}: {error_desc}")
    ...
    def get_credentials(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not stored_data:
                logger.debug("No stored credentials found for provider: %s", self.provider_name)
                return None
            ...
            try:
                ...
                if datetime.now() >= buffer_time:
                    logger.warning("Token for %s is expired or expiring soon", self.provider_name)
                ...
                logger.debug("Retrieved valid access token for provider: %s", self.provider_name)
                ...
                if stored_data.startswith('gho_') or stored_data.startswith('ghr_'):
                    logger.warning("Legacy token format detected for %s. Consider re-authenticating", self.provider_name)
                ...
                logger.debug("Stored data for %s is not a valid token format", self.provider_name)
                return None
        except Exception as e:
            logger.error("Failed to retrieve credentials for %s: %s", self.provider_name, str(e))
            return None
...
def get_credentials(provider_name: str, model_alias: str, verbose: bool = False) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not provider_data:
            logger.error("Provider '%s' not found in configuration for auth", provider_name)
        ...
        if verbose:
            logger.debug("Getting credentials for provider: %s, model: %s, auth_strategy: %s", 
                        provider_name, model_alias, auth_strategy_name)

        if not auth_class:
            logger.error("Unknown auth_strategy '%s' for provider '%s'", auth_strategy_name, provider_name)
            raise AuthenticationError(f"Unknown auth_strategy '{auth_strategy_name}' for provider '{provider_name}'")

        # Instantiate the auth strategy class
        if auth_strategy_name == "device_flow":
            auth_details = provider_data.get("auth_details", {})
            if verbose:
                logger.debug("Device flow details for %s: %s", provider_name, list(auth_details.keys()))
        ...
        if verbose:
            logger.debug("Retrieved credentials for %s: %s", provider_name, 'Yes' if credentials else 'None')
            if credentials:
        ...
        logger.error("Failed to get credentials for %s: %s", provider_name, str(e))
        raise 