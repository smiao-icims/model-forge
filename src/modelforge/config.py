...
def get_config_from_path(path: Path) -> Tuple[Dict[str, Any], Path]:
    ...
    try:
        ...
        logger.error("Invalid JSON in configuration file %s: %s", path, str(e))
        raise ConfigurationError(f"Invalid JSON in configuration file {path}") from e
    except IOError as e:
        logger.error("Could not read configuration file %s: %s", path, str(e))
...
def save_config(config_data: Dict[str, Any], local: bool = False):
    ...
    try:
        ...
        logger.error("Could not save config file to %s: %s", config_path, str(e))
...
def set_current_model(provider: str, model: str, local: bool = False):
    ...
    if provider not in providers or model not in providers[provider].get("models", {}):
        ...
        logger.error("Model '%s' for provider '%s' not found in %s configuration", model, provider, scope)
...
def migrate_old_config():
    ...
    if old_config_file.exists():
        if not GLOBAL_CONFIG_FILE.exists():
            ...
            logger.info("Old configuration file found, but new global configuration already exists")
